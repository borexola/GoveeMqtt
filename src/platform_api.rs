...
pub struct EnumOption {
    // keep the public API identical: a plain String field called `name`
    #[serde(deserialize_with = "deserialize_enum_option_name_graceful")]
    pub name: String,
    #[serde(default)]
    pub value: JsonValue,
    #[serde(flatten)]
...
/// Custom deserializer for `EnumOption.name`
///
/// It accepts both `name: "On"` and
/// `name: { "en": "On", "de": "Ein" }`.
/// When something unexpected arrives, it logs a warning and
/// falls back to "Unknown" instead of failing.
fn deserialize_enum_option_name_graceful<'de, D>(deserializer: D) -> Result<String, D::Error>
where
    D: serde::Deserializer<'de>,
{
    ...
    match v {
        // Normal case: API sends "name": "On"
        Value::String(s) => Ok(s),

        // Localised form: "name": { "en": "Game", "de": "Spiel", ... }
        Value::Object(map) => {
            if let Some(Value::String(en)) = map.get("en") {
                Ok(en.clone())
            } else if let Some((_k, Value::String(any))) =
                map.iter().find(|(_, v)| v.is_string())
            {
                Ok(any.clone())
            } else {
                log::warn!(
                    "EnumOption.name was an object but contained no strings; using fallback. content={:?}",
                    map
                );
        ...
        // Anything else: number, boolean, array, null…
        other => {
            log::debug!(
                "EnumOption.name had unexpected type {:?}; using fallback name.",
                other
            );
            Ok("Unknown".to_string())
        }
